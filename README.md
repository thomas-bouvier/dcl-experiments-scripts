...
If you have access to the Inria Gitlab, an image containing Horovod 0.24.1 + PyTorch 1.10.2 is available at `registry.gitlab.inria.fr/kerdata/kerdata-codes/horovod-images:0.24.1-gpu`. This container registry is protected by an access token, to be given in `layers_services.yml` files:
