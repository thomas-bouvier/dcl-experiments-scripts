...
#COBALT -M thomas.bouvier@inria.fr
...
mpiexec -np 64 -npernode 8 -hostfile $COBALT_NODEFILE -bind-to none -map-by slot -x NCCL_DEBUG=INFO -x LD_LIBRARY_PATH -x WANDB_MODE -x PATH python ../../main.py --yaml-config experiments_thetagpu.yaml --config baseline_naive_ghostnet_scale --log-level info